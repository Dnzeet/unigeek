...
export const CATALOG = [
  ...
  { slug: "karma-attack",         title: "Karma Attack",        category: "wifi",     summary: "Umbrella menu grouping the Karma tools (Captive, EAPOL, Detector, Support) plus Active Karma — an engine that answers directed probes as a fake AP, beacons a live SSID pool, and can raise a real open AP a victim can actually join; Passive/Broadcast/Full modes with an in-run options menu (channel, deauth, MAC rotation, response tiers, evil AP)", hasDetail: true,  stable: true },
  ...
  { slug: "wifi-analyzer",        title: "WiFi Analyzer",       category: "wifi",     summary: "Scan and display nearby networks with signal strength and channel info; select an AP to sniff its connected client MACs live (per-AP client list, promiscuous)", hasDetail: false, stable: true },
  ...
  { slug: "network-mitm",         title: "Network MITM",        category: "wifi",     summary: "Full-subnet ARP poisoning with real packet forwarding — capture victims' traffic to a PCAP that Wireshark reads already decrypted, no key setup", hasDetail: true,  stable: true },
  { slug: "dhcp-attack",          title: "DHCP Attack",         category: "wifi",     summary: "Starve the router's DHCP pool, deauth clients to force re-request, and serve rogue leases naming the device as gateway and DNS", hasDetail: true,  stable: true },
  ...
  { slug: "responder",            title: "Responder",           category: "wifi",     summary: "LLMNR/NBT-NS name poisoning + fake SMB that captures NetNTLMv2 hashes (hashcat -m 5600) to SD — under WiFi → Network; 8/16 MB boards only", hasDetail: true,  stable: true },
  { slug: "socks4-proxy",         title: "SOCKS4 Proxy",        category: "wifi",     summary: "Turn the device into a SOCKS4/4a TCP proxy on port 1080 so a laptop tunnels through its WiFi — under WiFi → Network; 8/16 MB boards only", hasDetail: true,  stable: true },
  ...
  { slug: "ble-device-spam",      title: "BLE Spam",            category: "ble",      summary: "Unified BLE advertising spammer (NimBLE) — Apple ProximityPair / NearbyAction / Not-Your-Device, Android Fast Pair, Windows Swift Pair, Samsung EasySetup, named beacon, and Random-All; selectable TX power and per-packet MAC randomization", hasDetail: false, stable: true },
  ...
  { slug: "ble-file-manager",     title: "BLE File Manager",    category: "ble",      summary: "Manage on-device files wirelessly from the browser over BLE (Nordic UART) — no WiFi; enable Bluetooth → Remote Device (advertises as UniGeek Remote); the same https://unigeek.xid.run/app/files page also connects over USB serial via HID → USB Remote", hasDetail: true,  stable: true },
  ...
  { slug: "sub-ghz",              title: "Sub-GHz (CC1101)",    category: "module",   summary: "Capture, replay, and jam Sub-GHz RF signals — 44 brand/manufacturer protocol decoders, fixed-code brute-force key sweep (9 protocols over RMT), peak frequency detector, live RAW/Code receive filter, continuous RAW waveform recorder (records until stop with live wave/RSSI animation), scrollable Signal Info view, KeeLoq auto-decode + counter-step (Replay +1) rolling-code replay; compatible with Flipper Zero .sub format", hasDetail: true,  stable: true },
  ...
  { slug: "nrf24",                title: "NRF24L01+",           category: "module",   summary: "2.4 GHz spectrum analyzer, constant-carrier channel-hopping jammer (10 band presets — WiFi, BLE, Bluetooth, Zigbee, USB, Video, RC… — live mode switching with Next/Prev, Sequential/FHSS hop), and MouseJack wireless keyboard injection", hasDetail: true,  stable: true },
  ...
  { slug: "remote-access",        title: "Remote Access",       category: "utility",  summary: "Mirror the device screen in a browser and control it over USB serial or Bluetooth — navigate, type, and tap from https://unigeek.xid.run/app/remote; needs HID → USB Remote (or Bluetooth → Remote Device) on", hasDetail: true,  stable: true },
  { slug: "android-app",          title: "Android Companion",   category: "utility",  summary: "Native Android app for Remote, File Manager, and Firmware Update over USB-OTG or Bluetooth — same wire protocol as the web tools; download the .apk from the Install page", hasDetail: true,  stable: true },
  ...
  { slug: "lua-runner",           title: "Lua Runner",          category: "utility",  summary: "Top-level main menu item; run Lua 5.1 scripts from SD with display + sprite graphics, button + touch input, modal text/number/select/confirm prompts, JSON, path/time/config helpers, SD CRUD, WiFi/HTTP, Sub-GHz RF (auto-detect CC1101 / M5 RF433), and toast notifications", hasDetail: true,  stable: true },
  ...
  { slug: "quenswer",             title: "Quenswer",            category: "game",     summary: "Two-player pass-the-device guessing — Player 1 sets a clue, hidden answer (≤20 chars) and attempt budget; Player 2 guesses letter by letter with correct placements locked in; harder answers/tighter budgets score more", hasDetail: false, stable: true },
  ...
  // Settings
  { slug: "setting-general",      title: "General Settings",    category: "setting",  summary: "Device name, display timeout, brightness, volume, navigation sound, theme color, WFM password, PIN, hidden modules, and device status (the USB Remote / Screen-mirror services moved to the HID menu)", hasDetail: true,  stable: true },
  ...
  { slug: "setting-hide-module",  title: "Hide Module",         category: "setting",  summary: "Show or hide individual entries in the Modules menu; hidden modules are persisted as a bitmask in config and stay hidden across reboots", hasDetail: false, stable: true },
